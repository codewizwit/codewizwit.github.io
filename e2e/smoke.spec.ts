...
const pages = [
  {
    path: "/",
    h1: "No Human Left Behind.",
    key: "Technology, ethics, and humanity evolving together.",
  },
  {
    path: "/about",
    h1: "The wiz who dove in sideways.",
    key: "AI Enablement Engineer",
  },
...
test("framework descriptions match between homepage and frameworks index", async ({
  page,
}) => {
  await page.goto("/");
  const homeLiftDesc = await page
    .locator(".fw-teaser-desc")
    .first()
    .textContent();
  const homeFourLensesDesc = await page
    .locator(".fw-teaser-desc")
    .nth(1)
    .textContent();
